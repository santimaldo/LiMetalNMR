...
es para obtener los parametros d, r y vs que si puedan ser simulados
"""
...
for distancia in distancias:
  for radio in radios:
    ...
    for vs in VSs:
      ...
      if h>Nz*0.6:
        #  debe haber espacio vacio entre los cilindros 
        msj = 'vs es muy chico en la direccion z'
        parametros_que_NO.append([distancia, radio, vs, msj])        
        continue
      if radio<vs:
        #  debe haber espacio vacio entre los cilindros 
...
datos = np.array([distanciasSI, radiosSI, vsSI, densidades]).T
file = "../DataBases/CilindrosAltaResolucion_parametros_d_r.dat"
header = r"distancias (um)\t radios (um)\t vs (um)\t densidad"
np.savetxt(file, datos, header=header, fmt='%.2f\t%.2f\t%.2f\t%.4f')
