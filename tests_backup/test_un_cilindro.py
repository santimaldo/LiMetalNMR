...
for semialtura in [50,5]:
  for  radio in [2,6,12,24]:
    ...
    for ind_z in range(int(Nz/2-semialtura), int(Nz/2+semialtura)+1):
      ...
      for ind_x in range(int(Nx/2-radio-1),int(Nx/2+radio+2)):
        for ind_y in range(int(Ny/2-radio-1),int(Ny/2+radio+2)):
          if (ind_x-Nx/2)**2+(ind_y-Ny/2)**2<r2:
    ...
    np.savetxt('S:/Doctorado/LiMetal/simulaciones/2021-06-10_superposicion_lateral/alcance_datos/altura{}_radio{}.dat'.format(int(semialtura*2+1),int(radio)), data)
  

#%%
plt.figure(88)
plt.pcolormesh(obj2d)
