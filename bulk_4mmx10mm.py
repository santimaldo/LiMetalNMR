...
d = delta[245:,25:39, 45:83]

Nz, Ny, Nx = np.shape(d)
z = np.linspace(0, (Nz-1)*VSz, Nz) - 0.7455
...
# la conclusion de esto es qe no vale la aprox d_out=cte para una lamina de estas
# dimensiones
plt.figure(222)
puntos = [[0,0], [3,0], [2,2], [3,5], [0,5], [2,7], [0,9], [3,9], [4,10 ]]
for punto in puntos:
  xind = punto[1]
  yind = punto[0]
  leyenda ='x='+str(x[xind])+',  y='+str(y[yind])
  plt.plot(z,d[:,yind,xind], '-o', label=leyenda)
plt.legend()
plt.xlabel('z [mm]')
plt.ylabel(r"$\delta$ [ppm]")
plt.xlim([-0.1,0.5])
...
#%%
#veamos d_in en la superficie para ver como seria el espectro

hist, bin_edges = np.histogram(d_in)
bins = bin_edges[0:-1] + 0.68578883/2

#%%
plt.figure(88)
#plt.plot(bins, hist, 'o-')
din = np.ndarray.flatten(d_in)
plt.hist(din, bins='auto')
plt.xlim(-8,-13)
