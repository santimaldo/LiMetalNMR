...
#densidades = [10,25,50,75,90]
alturas = np.array([50])*1e-3
anchos = np.array([1])*1e-3
...
for h in alturas:
  ...
  for ancho in anchos:
    ...
    for dens in densidades:
      ...
      # carpeta= 'centro/SP/'
      if seq=='smc':
        carpeta= 'centro_aleatorias_palo/SMC/'
    ...
    #  archivo = 'h{:d}_ancho{:d}_dens{:d}_SP_k0.50'.format(h,ancho,dens) 
      ...
      for region in regiones:
        if seq=='smc':
          ...
          archivo = 'centro{}_h{:d}_ancho{:d}_dens{:d}_SMC_N16_k{:.2f}'.format(region,h,ancho,dens,k)
        # extraigo  
...
#        plt.yticks([])      
  ...
  #  plt.show()
    ...
    #
    label = r'thickness: {:d} $\mu$m'.format(ancho)
...
#    plt.figure(n_a*100+n_h**2)
#    plt.title('altura: {}, ancho: {}'.format(h,ancho))
