...
alturas = np.array([25])*1e-3
anchos = np.array([10])*1e-3
porcentajes = np.array([10])
...
versiones = ['_b','c']
n_total = len(versiones)*len(alturas)
for n_ver in range(len(versiones)):
  version = versiones[n_ver]
  for nn in range(len(anchos)):
    ancho = anchos[nn]*1e-3
    h = alturas[nn]*1e-3
    porcentaje = porcentajes[nn]
    print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    msj = 'altura= {:d} um,  ancho= {:d} um,  densidad p= {:d}%'.format(int(h*1e3), int(ancho*1e3), porcentaje)
    print(msj)
    print(' ')
    progreso = ((n_ver)+(nn+1))/n_total*100
    print('         ... Progreso :  {:.2f}%  ({}/{})'.format(progreso, (n_ver)+(nn+1), n_total))
    ...
    shape = superposicion.muestra_sup.shape
    #slz, sly, slx = muestra.slices
    slz = [0, shape[0]]
    sly = [int(shape[1]/4-1),int(shape[1]*3/4+2)]
    slx = [int(shape[2]/4-1),int(shape[2]*3/4+2)]
    obj_0 = np.zeros([shape[0]+10,shape[1],shape[2]])
    obj_0[slz[0]:slz[1],:,:] = superposicion.muestra_sup*medicion.volumen_medido
    obj = obj_0[:,sly[0]:sly[1],slx[0]:slx[1]]
    tmpvol =  obj
    #%%
    filename = './stl/h{:d}_ancho{:d}_dens{:d}{}.stl'.format(int(h*1e3), int(ancho*1e3), int(porcentaje), version)
    tiempo = time.time()
    print('-------------convirtiendo a stl...')      
    with Oct2Py() as oc:
      fv = oc.isosurface(tmpvol, 0.5) # Make patch w. faces "out"
      oc.stlwrite(filename,fv)        # Save to binary .stl
    duracion = (time.time()-tiempo)/60
    print('el stl demora {} minutos'.format(duracion))
    #%% guardado
