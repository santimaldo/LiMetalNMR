...
path0 ='./Outputs/Resultados/aleatorios/'
...
for ii in range(radios.size):
  ...
  for jj in range(1,6):    
    ...
    for region in regiones:      
      # path    = path0 + 'SMC64-k1/'      
      archivo = 'h{:d}_r{:d}_d{:d}_iteracion{}{}.dat'.format(h, r, d, jj, region)
...
datos = np.array([alturas_t, radios_t, distancias_t, densidades_t, corrimientos, delta_mic, delta_bulk, amp_rel, iteraciones_t]).T
# np.savetxt(path0+'resultados.dat', datos)

# ordeno
datos = datos[datos[:,-1].argsort()] # First sort doesn't need to be stable. # caso
...
np.savetxt(path0+'resultados.dat', datos, fmt='%.4f', delimiter=',')
