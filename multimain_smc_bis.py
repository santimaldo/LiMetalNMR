...
import time
"""
COMENTTO TODO, ES SOLO PARA TENER BIEN EL PORCENTAJE CUBIERTO
"""
...
for ancho in anchos:
  for h in alturas:
    for porcentaje in porcentajes:
      ...
      p_Cubierto = muestra.pCubierto
      
      path = './Espectros/centro_aleatorias_palo/SP/pCubierto/'
      file = 'h{:d}_ancho{:d}_dens{:d}'.format(int(h*1e3), int(ancho*1e3), int(porcentaje))
      np.savetxt(path+file, np.array([p_Cubierto]))
