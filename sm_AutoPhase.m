function [res ph0]=sm_AutoPhase(S,ZF,pres,method,ppmAxis,ppm_range)
...
%         figure(314); plot(ppmAxis,real(Spec))
        newppm = ppmAxis;
        newppm(newppm<ppm_range(1))=[];
        newppm(newppm>ppm_range(2))=[];
        bordes = [find(ppmAxis==newppm(1)), find(ppmAxis==newppm(end))];
        for i=1:length(angle)
            Sp_try=Spec.*exp(-1i*angle(1,i)*pi/180);
            IntReSpec(1,i)=sum(real(Sp_try(bordes(1):bordes(2),1)));
        end
    ...
    case{'MinIntImagSpec'}
        ...
        for i=1:length(angle)
            Sp_try=Spec.*exp(-1i*angle(1,i)*pi/180);
            IntImagSpec(1,i)=sum(imag(Sp_try(bordes(1):bordes(2),:)));
        end
...
%         angulo=angle(1,indice);
    ...
    case{'MinIntImagFID'}
        ...
        for i=1:length(angle)
            ...
            IntImagFID(1,i)=trapz(imag(FID_try(1,:)));
        end
