...
def calculateFieldShift(voxelSize):
    """
...
#    end
    FOV = voxelSize.*np.array(np.shape(dChi_3D))
...
#        % referred to as the "DUAL" distribution:
        """
        Zc y D son funciones. Voy a traducir esas antes de seguir
        """
        FT_dChi_3D_DUAL = fftpack.fftn(dChi_3D + 1i*Zc(D(dChi_3D))); 
                clear dChi_3D;
...
####################################
####################################
X = np.random.random([8,8,8]) # borrar esto! es para probar
dim = np.shape(X)
